...
shinyServer(function(input, output, session) {
  ...
  ## > Tabeller >> Table ----
  output$tblTable <- renderDataTable({
    tblData()
  },
  
  # Options for the DataTables.js object that is generated by renderDataTable().
  # NOTE: IF WE WANT TO CREATE CHILD ROWS IN THIS TABLE, THIS IS WHERE TO PUT THE
  # CODE FOR THAT!
  options = list(
  ...
  geoData <- reactive({
    ...
    ppsDataGeo <- tbl_df(ppsData()) %>%
      ...
      left_join(coordinates %>% mutate(region = as.character(region)), by = "region") %>%
