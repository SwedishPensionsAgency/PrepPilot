...
Först en grundläggande typologisk förklaring: Shiny är ett _ramverk_, inte en _app_.
...
En programmerare har mycket lätt att komma igång med Shiny. Några dataförädlare och utvecklare på avdelningen har redan börjat använda ramverket för att lösa mindre uppgifter.
...
Shiny gör det lätt att göra fel. Det ställer betydligt högre krav på kompetens inom t.ex. design och visuell kommunikation.

En WYSIWYG-approach gör det möjligt att bara "sätta igång" med t.ex. en presentation eller liknande, medan en kodbaserad approach kräver ett visst mått av abstraktionsförmåga.

Interaktiv grafik i presentationer och rapporter kräver en R-serversession för att fungera. Det gör att portabiliteten i Shiny-produkter i vissa särskilda fall är beroende av en fungerande internetuppkoppling.
...
- Shiny är en plattform för frontend-kodning som bygger på att metoder i R används för att skapa en JavaScript- och HTML5-baserad slutprodukt. Detta innebär att en R-programmerare med lätthet kan utnyttja styrkan i flera kraftfulla .js-bibliotek som t.ex. DataTables, D3.js, Leaflet, ioSlides m.fl.
...
Inbyggd debugger, och visningsläge som visar vilken kod som körs vid en viss tidpunkt, gör utveckling och felsökning lätt och roligt.
...
- Dokumentationen innefattar såväl en välstädad API-dokumentation som lathundar och en bra tutorial, m.m.
...
- Körning direkt i R (kräver att R är installerat lokalt)
...
För att säkra åtkomst finns flera autentiseringslösningar inbyggda i Shiny Server:
...
Eftersom det går att köra Shiny-baserade applikationer lokalt genom R, kan vi redan idag använda Shiny för att utveckla ad hoc-rapporter som t.ex. denna presentation, skräddarsydda statistikrapporter eller liknande.

Det vi saknar är en driftsättningsplattform för kontinuerlig drift, men en sådan kan både ta formen av en molntjänst eller en in house-miljö.

Shiny utvecklas snabbt, och många nya features kan förväntas inom en nära framtid. Det bör dock understrykas att ramverket redan i sin nuvarande utformning är lättanvänt, elegant och förhållandevis stabilt.
